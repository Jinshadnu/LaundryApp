...
public class ItemAdapter extends RecyclerView.Adapter<ItemAdapter.ItemsViewViewHolder>  {
   ...
   // public ValueFilter valueFilter;
    ...
    public ItemAdapter(Context context, List<ServiceResponse.OurServices.Categorise.Item> itemsList) {
        ...
        this.mStringFilterList = new ArrayList<>(itemsList);
    }
...
//    }
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0)
            itemsList.addAll(mStringFilterList);
        else
            for (ServiceResponse.OurServices.Categorise.Item item : mStringFilterList)
                if (item.getItem_name().toLowerCase().contains(charText.toLowerCase()))
                    itemsList.add(item);
